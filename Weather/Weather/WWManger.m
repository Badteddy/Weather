...
@interface WWManger() <CLLocationManagerDelegate>



@end
@implementation WWManger
...
-(RACSignal *)updateCurrentConditions{
    return [[self.client fetchCurrentConditionsForLocation:self.currentLocation.coordinate] doNext:^( WWCondition *conditon) {
        self.currentCondition  = conditon;
    }];
}
-(RACSignal *)updateDailyForecast {
    return [[self.client fetchDailyForecastForLocation:self.currentLocation.coordinate]
            doNext:^(WWDailyForecast *forecast) {
                self.dailyForecast = forecast.list;
            }];
}
-(RACSignal *)updateHourlyforecast {
    return [[self.client fetchHourlyForecastForLocation:self.currentLocation.coordinate] doNext:^(WWHourlyForecast *forecast) {
