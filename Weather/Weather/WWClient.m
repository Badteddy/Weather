...
-(RACSignal *)fetchCurrentConditionsForLocation:(CLLocationCoordinate2D)coordinate {
    ...
    return [[self fetchJSONFromURL:url] map:^(NSDictionary *json) {
        return [MTLJSONAdapter modelOfClass:[WWCondition class] fromJSONDictionary:json error:nil];
    }];
}
-(RACSignal *)fetchHourlyForecastForLocation:(CLLocationCoordinate2D)coordinate {
    ...
    NSURL *url = [NSURL URLWithString:urlString];
    return [[self fetchJSONFromURL:url] map:^(NSDictionary *json) {
        return [MTLJSONAdapter modelOfClass:[WWHourlyForecast class] fromJSONDictionary:json error:nil];
    }];
}
...
-(RACSignal *)fetchDailyForecastForLocation:(CLLocationCoordinate2D)coordinate {
    ...
    return [[self fetchJSONFromURL:url] map:^(NSDictionary *json) {
        ...
        return [MTLJSONAdapter modelOfClass:[WWDailyForecast class] fromJSONDictionary:json error:nil];
    }];
}
